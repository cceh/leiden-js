...
function transformHi(elem: Element, output: string[]) {
    ...
    switch (rend) {
        ...
        case 'supraline':
            ...
            let hasUnclearElementChildren = false;
            let hasOtherElementChildren = false;

            for (const child of elem.children) {
                if (child.localName === 'unclear') {
                    hasUnclearElementChildren = true;
                } else {
                    hasOtherElementChildren = true;
                    break;
                }
            }

            if (hasOtherElementChildren) {
                output.push('¯');
                transformElem(elem, output);
                output.push('¯');
